...
| **Authorization Grant** | A credential representing the resource owner's consent. The client exchanges this grant for an access token. |
| **Access Token (Bearer)** | The secure key used by the client to access protected resources on behalf of the user. |
| **Resource Server** | The system hosting the protected user accounts and information. It verifies the access token before responding. |
...
| **Client ID & Secret** | The unique credentials used to identify and authenticate your application to the authorization server. |
...
> **Note:** Replace `<canvas-instance>` throughout this guide with the actual domain of your Canvas LMS instance (e.g., `canvas.instructure.com`).
