...
graph TD
    subgraph Ingestion ["Stage 1: Data Ingestion & Normalization"]
        TEI["TEI / XML Manuscript Corpus<br/>Accordance & Dead Sea Scrolls Archives"]
        Tokenizer["Consonantal Tokenizer<br/>Unicode Normalization & Strip Vowels"]
        Aligner["Multiple Sequence Aligner<br/>Needleman-Wunsch & Lacuna Handler"]
    end

    subgraph AnalyticsEngine ["Stage 2: Graph Clustering Engine"]
        DistCalc["Distance Matrix Processor<br/>Weighted Jaccard & Levenshtein Cost Matrix"]
        GraphClustering["Bayesian Stemmatic Engine<br/>DAG Calculation & Probability Scoring"]
        GraphDB["Neo4j / Graph Database<br/>Manuscript Nodes & Transmission Edges"]
    end

    subgraph PresentationLayer ["Stage 3: Interactive Apparatus UI"]
        GraphQL["GraphQL Apparatus API<br/>Targeted Locus Query Gateway"]
        ApparatusUI["Interactive Critical Apparatus<br/>SvelteKit / WebGL Dynamic Layout"]
    end
