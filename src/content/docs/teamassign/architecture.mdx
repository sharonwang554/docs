...
sequenceDiagram
    ...
    TA->>Canvas: Redirect to GET /login/oauth2/auth<br/>(client_id, redirect_uri, response_type=code, state)
    ...
    TA->>Canvas: POST /login/oauth2/token<br/>(grant_type=authorization_code, code, client_secret)
    ...
    Note over TA,Canvas: Subsequent API Calls automatically check pre-expiration (60s buffer)<br/>and execute check_and_update_token when expired!
```
