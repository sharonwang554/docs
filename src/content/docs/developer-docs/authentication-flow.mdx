...
For our core academic platform integration (**TMS**), OAuth 2.0 connects external Learning Management Systems (such as **Canvas LMS**) to our backend. When instructors link their profiles or import rosters, TMS acts as the OAuth **Client**, obtaining delegated credentials without handling the user's LMS password directly.

### Core OAuth 2.0 Terminology in TMS
...
- **Client**: The application requesting access on behalf of the resource owner (the **TMS** web platform).
...
- **Redirect URI**: The pre-registered TMS callback endpoint (`https://<dev_or_prod>.tms.org/faculty/my_profile`) where Canvas returns the user along with a temporary authorization `code`.

### 7-Step Authorization Sequence
1. **Initiation**: The instructor navigates to **My Profile** inside TMS and clicks **Link Canvas**.
2. **Redirect to Canvas**: TMS redirects the browser to the Canvas authorization endpoint, passing `client_id`, `response_type=code`, `redirect_uri`, and required `scope` strings (e.g., `url:GET|/api/v1/courses`).
3. **Instructor Consent**: If the instructor is not logged in, Canvas authenticates them. Canvas displays a consent prompt asking the instructor to authorize TMS for the requested scopes.
...
5. **Back-Channel Token Exchange**: TMS intercepts the code and executes a server-to-server `POST` request to `/login/oauth2/token` containing `client_id`, `client_secret`, `code`, and `redirect_uri`.
6. **Token Issuance**: Canvas returns a JSON payload containing the `access_token`, `refresh_token`, and `expires_in` (3600 seconds).
7. **Persistence**: TMS stores the tokens inside the `canvas_tokens` database table keyed to the instructor's `user_id`.
...
sequenceDiagram
    ...
    Instructor->>TMS: Click "Link Canvas" (My Profile)
    TMS->>Instructor: 302 Redirect to CanvasAuth (?client_id, &scope, &redirect_uri)
    Instructor->>CanvasAuth: Approve Scopes & Authenticate
    CanvasAuth->>TMS: 302 Redirect to /my_profile (?code=XYZ)
    TMS->>CanvasToken: POST /login/oauth2/token (client_id, client_secret, code)
    CanvasToken-->>TMS: JSON Response (access_token, refresh_token, expires_in)
    TMS->>DB: Store tokens & calculate expire_at timestamp
    TMS-->>Instructor: Render My Profile ("Canvas Linked Successfully")
```

### Source Code Architecture & Module Hierarchy (`lib/LMS/`)
The authorization logic inside the TMS repository (`tms-lib`) is structurally separated across controller actions and LMS-specific integration providers:
...
- **`TMS::Controller::Faculty->my_profile`**: Intercepts the incoming callback URL. When a `code` query parameter is detected during a GET request to `/my_profile`, the controller invokes `$lms->get_auth($code, $redirect_uri)` inside `Login.pm`.
