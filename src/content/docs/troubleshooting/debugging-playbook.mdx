...
```mermaid
graph LR
    A["Client Browser / SPA<br/>(React / SvelteKit)"] -->|POST /api/v1/sync<br/>Headers: X-Correlation-ID| B["AWS API Gateway /<br/>CloudFront Edge"]
    B -->|Event Payload<br/>requestContext.requestId| C["AWS Lambda Worker<br/>(Node.js / Express)"]
    C -->|SQL Query + Comment<br/>/* trace_id: req-8f92a */| D["Relational Database<br/>(RDS / MySQL / PostgreSQL)"]
    C -->|Async Publish<br/>MessageAttributes| E["AWS EventBridge /<br/>SQS Event Bus"]
...
When inspecting on-premise staging environments, legacy monolithic servers (such as [TeamAssign](/teamassign/overview)), or local Docker containers where interactive terminal access is available, use exact streaming shell forensics to isolate exceptions in real time:
