...
## Canvas LMS OAuth 2.0 Integration & Authorization Code Flow (C#/.NET Core Implementation)

For our core academic platform integration (**TMS**), OAuth 2.0 connects external Learning Management Systems (such as **Canvas LMS**) to our back-end. When instructors link their profiles or import rosters, TMS acts as the OAuth **Client**, obtaining delegated credentials without handling the user's LMS password directly.

### Core OAuth 2.0 Terminology in TMS
- **Resource Owner**: The entity granting access to protected data (for example, a **Canvas Instructor** authorizing access to course rosters and grades).
...
2. **Redirect to Canvas**: TMS redirects the browser to the Canvas authorization endpoint, passing `client_id`, `response_type=code`, `redirect_uri`, and required `scope` strings (for example, `url:GET|/api/v1/courses`).
...
```
tms-lib/lib/LMS/
├── LMS.cs          # Abstract base class defining common LMS interface methods
├── Login.cs        # Core OAuth 2.0 handshake, token exchange, and refresh handler
├── Canvas.cs       # Canvas-specific API client (courses, sections, SIS imports)
└── Utils.cs        # HTTP user-agent formatting, payload signing, and error logging
```

- **`TMS::Controller::Faculty->my_profile`**: Intercepts the incoming callback URL. When a `code` query parameter is detected during a GET request to `/my_profile`, the controller invokes `$lms->get_auth($code, $redirect_uri)` inside `Login.cs`.
