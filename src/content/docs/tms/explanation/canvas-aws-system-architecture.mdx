...
graph TB
    subgraph ClientLayer ["Client & LTI Integration Layer"]
        Canvas["Canvas LMS Cloud\nLTI 1.3 Advantage / REST API"]
        Browser["Faculty & Student Clients\nWeb Browsers & Mobile Apps"]
    end

    subgraph SecurityLayer ["API Gateway & Security Layer"]
        WAF["AWS WAF & Shield\nDDoS & Rate Limiting"]
        ALB["Application Load Balancer\nTLS Termination & Routing"]
        Cognito["AWS Cognito / LTI Auth Gateway\nJWT Validation & FERPA Roles"]
    end

    subgraph ComputeLayer ["Compute & Microservices Cluster (EC2 / ECS)"]
        IngestionSvc["Ingestion Microservice\nNode.js / Express Gateway"]
        AnalyticsSvc["Analytics Aggregator\nPython / FastAPI Engine"]
        GradePassbackSvc["Grade Passback Worker\nGo / Event Processor"]
    end

    subgraph MessagingLayer ["Asynchronous Decoupling Layer"]
        SQS["AWS SQS Event Queues\nGrade Passback & Telemetry Topics"]
        DLQ["AWS Dead-Letter Queue (DLQ)\nFailed Message Isolation"]
    end

    subgraph StorageLayer ["Persistence & Storage Layer"]
        RDS["Amazon RDS PostgreSQL\nMulti-AZ Relational Roster & Grades"]
        DynamoDB["Amazon DynamoDB\nSession State & Idempotency Cache"]
        S3["Amazon S3 Data Lake\nEncrypted Audit Logs & Export Archives"]
    end
