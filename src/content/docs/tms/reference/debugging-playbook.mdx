...
```json
{
  ...
  "error": {
    ...
    "message": "Timestamp drift exceeded maximum allowable window (300 s),"
...
<Tabs>
  ...
  <TabItem label="High-Speed Pattern Hunting (`ripgrep`)">
    ...
    ```bash
    rg "req-8f92a4c1-9b3e-cd1a-8c0e-7a2b919e3c11" /var/log/app/ --no-heading --line-number
    ```
  </TabItem>
...
3. Enter the exact domain name (for example, `local.raisedchurch.com` or `dev-api.internal`) into the `Domain:` input field and click **Delete**.
